...
import { createPremiumController, type PremiumController } from './premiumController';
import { buildFreePayload, buildPremiumPayload } from '../../data/intakeSchema';
import type { FreeIntakeData, PremiumIntakeData, Sex, TraitAnswers, PastSportInput } from '../../data/intakeSchema';
...
export type PastSportsEntry = PastSportInput & { id: string; sport_label?: string | null };
...
const STEP_META: Record<StepKey, { title: string; subtitle: string }> = {
  ...
  measurements: { title: 'Measurements', subtitle: 'Body, torso & limb inputs' },
...
const isSexOption = (value: string): value is Sex => SEX_OPTIONS.has(value as Sex);
...
const IntakeApp: FunctionalComponent<IntakeAppProps> = ({ mode }) => {
  ...
  const [sportySnapshot, setSportySnapshot] = useState<SportySnapshot>({ user: null, hasConsent: false });
  const [pastSports, setPastSports] = useState<PastSportsEntry[]>([]);
  const [basics, setBasics] = useState<{ birthday: string; sex: Sex | '' }>({ birthday: '', sex: '' });
  ...
  const activePremiumSection =
    mode === 'premium'
      ? PREMIUM_SECTION_KEYS.find((section) => premiumSectionIndexMap.get(section) === currentStepIndex) ?? null
      : null;
  const showPremiumBlock = activePremiumSection !== null;

  const submitLabel = 'See my matches';
  const setStatus = useCallback((html: string, type: 'info' | 'error' = 'info') => {
  ...
  const restoreDraft = useCallback(() => {
    ...
    try {
      ...
      if (Array.isArray(parsed.pastSports)) {
        const restoredPastSports = parsed.pastSports.map((entry: any, idx: number) => ({
          ...entry,
      ...
      if (legacyFields && typeof legacyFields === 'object') {
        const newBasics = { birthday: legacyFields.birthday || '', sex: legacyFields.sex || '' };
        setBasics(newBasics);
      ...
      const stepValue = typeof parsed?.step === 'number' && parsed.step > 0 ? parsed.step : null;
      if (stepValue) {
        const index = Math.max(0, Math.min(stepDefinitions.length - 1, stepValue - 1));
    ...
    if (import.meta.env.DEV) {
      try {
        if (!window.localStorage.getItem(STORAGE_KEY)) {
          setBasics({ birthday: '1990-01-01', sex: 'female' });

          const dummyMeasurements: Record<string, number> = {};
          measurementFields.forEach((f) => {
            // Use a value in the middle of the range or a default
            const min = f.min ?? 0;
            const max = f.max ?? 100;
            dummyMeasurements[f.id] = Math.round((min + max) / 2);
          });
          setMeasurements(dummyMeasurements);
  ...
  const validateBasics = useCallback((): StepValidationResult<Pick<FreeIntakeData, 'birthday' | 'sex'>> => {
    if (!basics.birthday) {
      return { ok: false, message: 'Add your birthday before continuing.', elementId: 'birthday' };
    }
    if (!basics.sex || !isSexOption(basics.sex)) {
      return { ok: false, message: 'Select the sex assigned at birth before continuing.', elementId: 'sex' };
    }
    return { ok: true, data: { birthday: basics.birthday, sex: basics.sex as Sex } };
  }, [basics]);

  const validateMeasurements = useCallback((): StepValidationResult<Pick<FreeIntakeData, typeof measurementKeys[number]>> => {
    const data: Partial<Pick<FreeIntakeData, typeof measurementKeys[number]>> = {};
    ...
    for (const id of measurementKeys) {
      ...
      if (value === null || value === undefined || value === ('' as any)) {
        return { ok: false, message: `Enter ${label.toLowerCase()} before continuing.`, elementId: id };
      }

      const numVal = Number(value);
      if (!Number.isFinite(numVal)) {
        return { ok: false, message: `${label} must be a number.`, elementId: id };
      }

      if (typeof field?.min === 'number' && numVal < field.min) {
        return { ok: false, message: `${label} must be at least ${field.min}.`, elementId: id };
      }

      if (typeof field?.max === 'number' && numVal > field.max) {
        return { ok: false, message: `${label} must be at most ${field.max}.`, elementId: id };
      }
      data[id] = numVal;
    }
  ...
  const handleSubmit = useCallback(
    async (event?: Event) => {
      ...
      const premiumSelection =
        premiumController?.collect() ?? { applyCredit: false, data: null, errors: [] };
      ...
      if (requiresPremium && !premiumSelection.applyCredit) {
        setStatus('Apply an adult analysis credit before submitting this premium intake.', 'error');
        return;
      }
      ...
      const traitAnswers: TraitAnswers = wantsPremium ? collectTraitAnswers() : {};
      ...
      setSubmitBusy(true);
      setStatus(wantsPremium ? 'Applying your credit and crunching the numbers…' : 'Crunching the numbers…', 'info');
      ...
      try {
        const endpoint = wantsPremium ? '/api/recommend-adult-premium' : '/api/recommend-adult-free';
        ...
        const storageKey = wantsPremium ? 'sporty:lastPremiumResult' : 'sporty:lastResult';
        ...
        setStatus((error as Error).message || 'Unexpected error, please try again.', 'error');
        setSubmitBusy(false);
      }
    },
    [collectTraitAnswers, focusField, mode, setSubmitBusy, setStatus, validateBasics, validateMeasurements, sportySnapshot, pastSports, basics, measurements]
  );
  ...
  useEffect(() => {
    ...
    const premiumController = createPremiumController({
      ...
      getClient: () => (typeof window !== 'undefined' ? (window as any).SportyApp?.getClient?.() ?? null : null),
    });
    ...
    if (sportyApp && sportyApp.ready) {
      sportyApp.ready.then(() => {
        if (typeof sportyApp.onAuthChange === 'function') {
          sportyApp.onAuthChange((snapshot: SportySnapshot | null) => applySnapshot(snapshot));
        } else {
        ...
        {!isFinalStep && (
          <button
            ...
            onClick={() => {
              ...
              setStatus('');
              const nextIndex = Math.min(stepDefinitions.length - 1, currentStepIndex + 1);
    ...
    <div className="flex flex-col gap-6">
      <div className="grid gap-3 md:grid-cols-3" data-stepper-nav>
        {stepDefinitions.map((step, idx) => {
          ...
          const activeClasses = isActive ? 'border-2 border-slate-900 shadow-none' : '';
          const completedClasses = isCompleted ? 'bg-teal-50 border-teal-400 text-teal-800' : '';

          return (
            <button
              key={step.key}
              type="button"
              className={[baseClasses, activeClasses, completedClasses].filter(Boolean).join(' ')}
            ...
            >
              ...
              <span className="flex flex-col items-start">
                ...
                <span className="type-small text-slate-500">{step.subtitle}</span>
              </span>
            </button>
          );
        })}
            ...
            switch (step.key) {
              ...
              case 'measurements':
                return (
                  <MeasurementsStep
                    values={measurements}
                    onChange={(patch) => setMeasurements((prev) => ({ ...prev, ...patch }))}
                  />
                );
              case 'traits':
                return <TraitsStep />;
              case 'pastSports':
                return <PastSportsStep entries={pastSports} onUpdate={setPastSports} />;
              default:
                return null;
            }
          })();
