  ...
  return (
    <div className="card-shell space-y-6" data-step="past-sports">
      <header className="space-y-2">
        <h2 className="type-title text-slate-900">Past sports</h2>
        <p className="text-slate-600">
          Add as many sports as you want. Signed-in users blend this into future
          matches.
        </p>
      </header>
          ...
          <input
            ...
            onInput={(e) => {
              ...
              if (entry.sport_subcategory_id) {
                onChange({
                  sport_subcategory_id: null,
                  sport_label: e.currentTarget.value,
                });
              } else {
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="space-y-2">
          <span className="type-body flex items-center gap-2 font-semibold text-slate-800">
            Years played
            <InlineInfoTip
              id={`${entry.id}-years-tip`}
              label="Years played"
              steps={[
                "Enter total active years in this sport.",
                "Subtract long breaks from the total.",
              ]}
            />
          </span>
          <NumberStepper
            ...
            onChange={(val) =>
              onChange({
                years_played: val === "" || val === null ? null : Number(val),
              })
            }
        ...
        <div className="space-y-2">
          <span className="type-body flex items-center gap-2 font-semibold text-slate-800">
            ...
            <InlineInfoTip
              id={`${entry.id}-age-tip`}
              label="Starting age"
              steps={["Enter the age when you first started this sport."]}
            />
          </span>
          <NumberStepper
            step={1}
        ...
        <label className="block space-y-2">
          <span className="type-body flex items-center gap-2 font-semibold text-slate-800">
            Intensity
            <InlineInfoTip
              id={`${entry.id}-intensity-tip`}
              label="Intensity"
              steps={[
                "Pick how hard you trained relative to your potential in this sport.",
              ]}
            />
          </span>
          <select
