...
1. Worker serves static assets (`site/`) and routes pretty URLs to pages.
...
- Worker secrets: `BACKEND_URL`, `BACKEND_API_KEY`, `STRIPE_PUBLIC_KEY`, `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY`.
...
- `POST /api/recommend-adult-free` (Worker) → `/recommend-adult-free` (Backend)
  ...
  - Response: top three matches including sport metadata from Supabase, role hierarchy descriptions, AI rationale, example athletes, and per-metric scores. Only `optimal_bodies.is_current = true` rows are returned.
